1. What are some examples of instructions handled by the CPU (as opposed to the ALU)?
   CPU - And, or, not, xor, nor, nand

ALU handles math and is part of the CPU

ALU handles math, and CPU tell it when to do it

2. Why are general-purpose registers useful in assembly language?

They are fast and can store temporary data.

3. Convert the 8-bit hex number 0xB7 to binary.

127 B is 12

4.  Use bitwise operations to turn off the 5th bit from the right of 0b01111100.

        01111100

    and 01101111

5) Why do people tend to gravitate towards base-10 number systems while computers do better with base-2  
   People have 10 fingers. Computers use binary at the lowest level because it boils down to off or on in transistors.

6) Create an AND mask to preserve the last 3 right-hand bits of any 8-bit number.

7. Describe how RAM is better than registers, and vice-versa.

Ram can hold a lot more data, but registers run the data they have faster

8. Describe the difference in what happens when a JUMP command is executed versus a CALL command

Jump goes to a location without doing anything else, CALL pushes the current pointer on the stack then JMP's to the location.
